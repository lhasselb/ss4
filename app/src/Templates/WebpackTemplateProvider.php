...
class WebpackTemplateProvider implements TemplateGlobalProvider
{
    ...
    /**
     * Calculate the path to webpack dev server
     * or Silverstripe public after checking if webpack is active
     *
     * @param String $path
     * @return void
     */
    ...
    {
        //Injector::inst()->get(LoggerInterface::class)->debug('WebpackTemplateProvider - _getPath for ' . $path . ':');
        return self::isActive() && strpos($path, '//') === false && strpos($path, 'thirdparty') === false ?
